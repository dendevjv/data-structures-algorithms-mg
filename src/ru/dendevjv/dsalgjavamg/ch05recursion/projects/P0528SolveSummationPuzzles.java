...
public class P0528SolveSummationPuzzles {
    ...
    private static class Puzzle {
        ...
        private boolean isExpressionChar(char ch) {
            boolean digitChar = Character.isDigit(ch);
            boolean operatorChar = "+-*/=".indexOf(ch) > -1;
            boolean space = ch == ' ';
            return digitChar || operatorChar || space;
        }
    }
    ...
    private static void solvePuzzle(String puzzle) {
        ...
        enumerateConfigurations(puzzle, DECIMAL_DIGITS);
    }
    
    

    private static void enumerateConfigurations(String conf, String digits) {
        if (new Puzzle(conf).isCorrect()) {
            System.out.println(conf + " is correct");
        } else {
            ...
            if (letterIndex > -1) {
                for (int i = 0; i < digits.length(); i++) {
                    ...
                    enumerateConfigurations(nextConf, newDigits);
                }
            }
        }
    }
